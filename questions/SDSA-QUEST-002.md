...
- **Decision required:** How a business owner configures one booking provider for the organization while each coach maps to that shared provider's user, resource, calendar, event type, or route.
- **Recommendation:** Use academy-owned coach pages backed by a simple managed coach-content system and one owner-selected booking adapter per business. Keep bios, videos, publication status, availability status, and calls to action under the business brand. Let the selected provider handle calendar synchronization, booking, rescheduling, cancellation, and notifications for all of that business's coaches. Route `Request a callback` through the separately verified human workflow.
...
- **Human decision:** Use a flexible, provider-neutral prototype configured once per business. Cal.com is only an example. A business owner may connect Google Calendar appointment scheduling, Calendly, Cal.com, another supported provider, or callback-only mode. Every coach in that business uses the same engine with a coach-specific provider mapping; individual coaches do not choose separate booking engines. Build the successful prototype experience and a clean integration contract; a separate agent will connect it to the existing CRM platform. Coach and booking-provider management is admin-only and must not be exposed through the public marketing site.
